...
@Component({
  ...
  styleUrls: ['./add-invest.component.less']
})
export class AddInvestComponent implements OnInit {
  ...
  ngOnInit() {
    this.invest = this.investService.makeEmptyInvest()
  ...
  makeContactsFormGroup(): FormArray {
    ...
    // arr.push(
    //   this.makeGroupOneContact()
    // )
    return arr
  }

  makeGroupOneContact(): FormGroup {
    return this.fb.group({
      who: [''],
      name: [''],
      email: [''],
  ...
  saveForm() {
    ...
    if (this.investForm.valid) {
      ...
      this.investService.createInvest(invest, this.invest, this.headerCroppedImage, this.horizontCroppedImage, this.mapCroppedImage).then(r => {
